export { getBankTransferTableConfig } from './table/get-bank-transfer.config';
export { BANK_TRANSFER_ACTION_CONFIG_MAP } from './dialog/get-bank-transfer.config';
export {
  ADD_BANK_TRANSFER_DEFAULT_TDS_PERCENT,
  ADD_BANK_TRANSFER_FORM_CONFIG,
} from './form/add-bank-transfer.config';
export { EDIT_BANK_TRANSFER_FORM_CONFIG } from './form/edit-bank-transfer.config';
export { SEND_EMAIL_BANK_TRANSFER_FORM_CONFIG } from './form/send-email-bank-transfer.config';
