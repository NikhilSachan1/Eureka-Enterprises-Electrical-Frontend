...
} from '@features/settings-management/permission-management/system-permission-management/config/dialog/system-permission-dialog.config';
import {
...
export class SystemPermissionListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.table = this.dataTableService.createTable(
      SYSTEM_PERMISSION_LIST_ENHANCED_TABLE_CONFIG as IEnhancedTableConfig
    );
  ...
  private mapTableData(
    response: IGetSystemPermissionListResponseDto
  ): Partial<IGetSingleSystemPermissionListResponseDto>[] {
    return response.records.map(
      (record: IGetSingleSystemPermissionListResponseDto) => ({
        id: record.id,
        name: record.name,
    ...
    switch (actionType) {
      case EBulkActionType.DELETE:
        this.showBulkDeleteConfirmationDialog(
          selectedRows as IGetSingleSystemPermissionListResponseDto[]
        );
        break;
      default:
